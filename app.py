...
this_endpoint_ip = sys.argv[1]
other_endpoint_ip = sys.argv[2]
...
def pullCompleted():
    ...
    if len(completed_work_q) < top:
        try:
            req = requests.get(f'http://{other_endpoint_ip}:{5000}/getCompletedWorkQ')
...
def createNewWorker():
    ...
    instance = ec2Resource.create_instances(
        ...
        KeyName="cloud-course-ex2",
        UserData=f"""#!/bin/bash
                set -e -x
                sudo apt update
                sudo apt install git -y
                sudo pwd
                sudo git clone https://github.com/NoyAvargil/Ex2.git
                sudo cp Ex2/worker.py /home/ubuntu/worker.py
                sudo apt install python3 -y
                sudo apt install python3-pip -y
                sudo pip install boto3
                # run app
                nohup python3 Ex2/worker.py $this_endpoint_ip $other_endpoint_ip &>/dev/null &
                exit
                """
        )
